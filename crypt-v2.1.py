...
DBUG_V = False # verbose debug mode (detailed outputs, near line-by-line operation)
numArray = []
...
def ranNumST(dLen=128, itCount=1000, tlrnc=2) -> None:
    ...
    if(DBUG):
        ...
        print(f"{CBLU}Analysis Complete: Test Results Below.{CEND}")
        print(f"{sum(numDist)}/{dLen*itCount} | CHKSUM..........{CGRE+'PASS'+CEND if (sum(numDist) == dLen*itCount) else CRED+'FAIL'+CEND}")
            
    # hand-written consistency check; will replace above
